...
// Recalcula a previsão sempre que o tipo ou a quantidade mudarem
...
// CRITÉRIO DE DIAS: MTO = 5 dias | Etiquetagem = 7 dias
function calcularPrevisaoEspecifica(tipoMaterial, quantidade) {
    ...
    // Regra: MTO = +5 dias | Etiquetagem / Outros = +7 dias
    const diasAdicionais = (tipoNormalizado === "MTO") ? 5 : 7;
...
function renderTabela() {
    ...
    filtradas.forEach(item => {
        ...
        <tr>
            ...
            <td><span class="badge ${item.status ? item.status.toLowerCase() : 'pendente'}">${item.status || 'PENDENTE'}</span></td>
            <td>${botoesAcao}</td>
        </tr>`;
    });
...
function atualizarKPIs(dados) {
    ...
    document.getElementById("kpiFinalizados").innerText = dados.filter(x => x.status === "FINALIZADO").length;
    document.getElementById("kpiAguardando").innerText = dados.filter(x => x.status === "AGUARDANDO").length;
}
...
function abrirModal(docId) {
    ...
    document.getElementById("areaPcp").value = item.areaPcp !== "-" ? item.areaPcp : "Montagem";
...
window.filtrarMes = () => { filtroMesAtual = document.getElementById("filtroMes").value; renderTabela(); };
window.limparFiltro = () => { filtroMesAtual = ""; document.getElementById("filtroMes").value = ""; renderTabela(); };
window.toggleSelecionarTodos = (m) => { document.querySelectorAll(".chk-solicitacao-item").forEach(c => { c.checked = m.checked; gerenciarSelecaoItem(c); }); };
window.gerenciarSelecaoItem = (c) => { 
    if (c.checked) { if (!solicitacoesSelecionadasIds.includes(c.value)) solicitacoesSelecionadasIds.push(c.value); } 
    else { solicitacoesSelecionadasIds = solicitacoesSelecionadasIds.filter(x => x !== c.value); }
};